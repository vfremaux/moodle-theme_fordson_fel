 ...
 */
function theme_fordson_fel_get_dynamic_css($theme) {
    global $PAGE, $USER;
    ...
    if (!empty($theme->settings->usedyslexicfont)) {
        if (get_user_preferences('dyslexic_helper', false, $USER->id)) {
            ...
            if (empty($fonturl)) {
                $fonturl = new moodle_url('/theme/fordson_fel/fonts/opendyslexic3-regular-webfont.woff');
                $defaultfamily = 'mainfont';
    ...
    if (get_user_preferences('highcontrast_helper', false, $USER->id)) {
        ...
        if (empty($fonturl)) {
            $fonturl = new moodle_url('/theme/fordson_fel/fonts/Kanit-Medium.ttf');
            $defaultfamily = 'mainfont';
            $titledefaultfamily = 'mainfont';
        }
    }

    $csscontent = theme_fordson_fel_load_uncached_css();
...
function theme_fordson_fel_load_uncached_css($stylesheetsoptions = []) {
    global $PAGE, $CFG;

    $css = '';
    $stylespath = $CFG->dirroot.'/theme/fordson_fel/uncached_style/*.css';
